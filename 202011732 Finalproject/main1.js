...
let top1=200;                        //top은 각 기둥에 가장 위에 있는 원반의 top값을 의미합니다.
let top2=450;
let top3=450;
    ...
    if (goal){}   //성공하면 클릭해도 반응하지 않도록 한다.
        ...
        if (len1>1)      //기둥에 원반이 있을 때만 적용된다.
        {
            if (peg1.classList.contains('selected')){        //한번 눌리면 원반이 들리며 selected가 생기고 한번 더 눌리면 원반을 제자리에 내려놓고 selected를 없앤다.
                ...
                peg1.children[len1-1].style.cssText='top: 30px';
            }
        }
}
...
function move_1To2(){
    ...
    if (peg3.children.length>5){              //3번째 기둥에 모두 원반이 모이면 goal이 true로 바뀜
